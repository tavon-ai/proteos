...
// Reconnect banner that re-runs the Connect GitHub flow (Phase 7 decision #8;
// since TAV-149 login is Zitadel, so only the connect flow refreshes the GitHub
// grant). The distinction is the one place GitHub-App auth is user-visibly
// different from a classic OAuth token, so surfacing it beats "why did my git
// stop working?".
  ...
  if (reconnect) {
    return (
      <div className="github-reconnect" role="alert">
        ...
        <a className="btn-primary" href={githubConnectUrl}>
          Reconnect GitHub
        </a>
      </div>
    );
